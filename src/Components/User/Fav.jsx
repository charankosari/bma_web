...
const Fav = () => {
  ...
  const handleDoctorCardClick = async (doctor) => {
    try {
      const jwtToken = localStorage.getItem('jwtToken');
      const response = await fetch(`https://server.bookmyappointments.in/api/bma/hospital/hospital/${doctor.hospitalid}`, {
        headers: {
          'Authorization': `Bearer ${jwtToken}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleTestClicked = async (test) => {
    try {
      const jwtToken = localStorage.getItem('jwtToken');
      const response = await fetch(`https://server.bookmyappointments.in/api/bma/hospital/hospital/${test.hospitalid}`, {
        headers: {
          'Authorization': `Bearer ${jwtToken}`,
          'Content-Type': 'application/json',
        },
